...
- `MEETING_TYPES = ("1on1","retro","skip-level","design-review","standup","interview","other")`; validated by `parse_meeting_type` (lowercased)
...
- 205 tests collected (204 offline + 1 smoke). All offline tests need no mic or model.
...
- **`.raw` audio deleted by default** on `finalize_meeting` unless `keep_audio=True`; `record --keep-audio` writes a 16kHz mono WAV instead.
